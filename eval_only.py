...
from models import EncoderRNN, DecoderRNN, S2VTAttModel, S2VTModel, CTCmodel
...
def convert_data_to_coco_scorer_format(data_frame):
    ...
    for row in zip(data_frame["caption"], data_frame["video_id"]):
        if not ((int(row[1][5:]) >= 0) and (int(row[1][5:]) <= 9999)):
            continue
        if row[1] in gts:
            gts[row[1]].append(
...
def test(model, crit, dataset, vocab, opt):
    ...
    for data in loader:
        ...
        with torch.no_grad():
            ...
            out_put = model(fc_feats)
        out_put = F.log_softmax(out_put, dim=2)
        # sents = utils.decode_sequence(vocab, seq_preds)
        _, preds = out_put.max(2)
        for i in range(preds.shape[0]):
            print(preds[i, :])

        sents = utils.decode_sequence(vocab, preds)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--saved_model', type=str, default='save/no_softmax/model_0.pth',
