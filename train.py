...
from models import DecoderRNN, EncoderRNN, S2VTAttModel, S2VTModel ,Two_Lstm
...
def train(loader, model, crit, optimizer, lr_scheduler, opt, rl_crit=None,opt_test=None,test_dataset=None):
    model.train()
    loss_avg = averager()
    ...
    for epoch in range(opt["epochs"]):
        ...
        if epoch % opt["save_checkpoint_every"] == 0:
            test(model,crit,test_dataset,test_dataset.get_vocab(),opt_test,writer)
            model.train()
